use super::contexts::SnarkContext;
...
impl SnarkProver {
    ...
    pub fn prove(&self, ctx: SnarkContext) -> anyhow::Result<(bool, Vec<u8>)> {
        ...
        let compress_proof = prover.shrink(ctx.0, network_prove.opts)?;
        let outer_proof = self.wrap_bn254(&prover, compress_proof, network_prove.opts)?;
        ...
        Ok((true, serde_json::to_vec(&ZKMProof::Groth16(proof))?))
    }
