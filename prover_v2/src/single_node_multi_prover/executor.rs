...
use super::contexts::SplitContext;
use crate::{
...
use zkm_prover::{CoreSC, InnerSC, ZKMProver};
...
impl Executor {
    pub fn split(
        &self,
        ctx: &SplitContext,
        records_tx: Arc<Mutex<SyncSender<Vec<ExecutionRecord>>>>,
    ) -> anyhow::Result<(u64, u32, Vec<ZKMDeferredWitnessValues<InnerSC>>)> {
        // todo: use new prover
        ...
        let (total_steps, total_segments, public_values_stream, deferred_proofs) = self
            .split_with_context(
                &prover,
                program,
                vk,
        ...
        Ok((total_steps, total_segments, deferred_proofs))
    }
    ...
    pub fn split_with_context<'a>(
        ...
        records_tx: Arc<Mutex<SyncSender<Vec<ExecutionRecord>>>>,
    ) -> anyhow::Result<(u64, u32, Vec<u8>, Vec<ZKMDeferredWitnessValues<InnerSC>>)> {
        ...
        std::thread::scope(move |s| {
            ...
            let total_segments = Arc::new(Mutex::new(0));
            ...
            let mut deferred_witness_values = vec![];
                        ...
                        loop {
                            ...
                            if let Ok((index, mut checkpoint, done)) = received {
                                ...
                                *report_aggregate.lock().unwrap() += report;
                                ...
                                let mut total_segments = total_segments.lock().unwrap();
                                *total_segments += records.len();

                                // Let another worker update the state.
                                record_gen_sync.advance_turn();

                                // Send the generated records.
                                records_tx
                                    .lock()
                                    .unwrap()
                                    .send(records)
                                    .expect("failed to send generated records");
                                ...
                                if done && !stdin.proofs.is_empty() {
                                    ...
                                    deferred_witness_values = prover.get_recursion_deferred_inputs(
                                        vk,
                                        last_proof_pv,
                                        &deferred_proofs,
            ...
            let total_segments = {
                let total_segments = total_segments.lock().unwrap();
                *total_segments
            };
